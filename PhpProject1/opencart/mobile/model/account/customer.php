...
class ModelAccountCustomer extends Model {
	...
	$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET code = '" . $this->db->escape($code) . "' WHERE `telephone` = '" . $mobile . "'");
    }
	...
	$query = $this->db->query("SELECT count(customer_id) as  count  FROM " . DB_PREFIX . "customer WHERE telephone = $telephone");
	return $query->row;
    }

    public function isUserMobile($customer_id, $telephone) {
	$query = $this->db->query("SELECT count(customer_id) as  count  FROM " . DB_PREFIX . "customer as c WHERE  c.customer_id=$customer_id and c.telephone=$telephone");
	return $query->row;
    }
